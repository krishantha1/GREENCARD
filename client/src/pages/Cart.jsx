        ...
        <div className="flex flex-col md:flex-row mt-16 ">
            <div className='flex-1 max-w-4xl'>
                ...
                {cartArray.map((product, index) => (
                    <div key={index} className="grid grid-cols-[2fr_1fr_1fr] text-gray-500 items-center text-sm md:text-base font-medium pt-3">
                        <div className="flex items-center md:gap-6 gap-3">
                            <div  onClick={()=>{
                                navigate(`/products/${product.category.toLowerCase()}/${product._id}`); screenTop(0,0)
                ...
                <button  onClick={()=> {navigate("/products"); scrollTo(0,0)} } className="group cursor-pointer flex items-center mt-8 gap-2 text-indigo-500 font-medium">
                   <img className="group-hover:translate-x-1 transition" scr={assets.arrow_right_icon_colored} alt="arrow"></img>
                    Continue Shopping
                </button>
            ...
            <div className="max-w-[360px] w-full bg-gray-100/40 p-5 max-md:mt-16 border border-gray-300/70">
                ...
                <button onClick={placeOrder} className="w-full py-3 mt-6 cursor-pointer bg-indigo-500 text-white font-medium hover:bg-indigo-600 transition">
                    {paymentOption == "COD" ? "Place Order" : "Proceed to Checkuot"}
                </button>
            </div>
        </div>
    ): null
